...
function StatsSection_1({className, stats}: Props) {
  return (
    <div className={"w-full bg-black/95 py-36 grid grid-cols-4 gap-6 place-items-center " + className} >
        {
        stats?.map((stat, i) => (
            ...
            </p>
            {
                ...
                ?
                <AnimatedStatCount speed={stat.speed} start={stat.start} end={stat.end} counterClassName="!text-9xl !font-bold !text-slate-300/40" />
                :
                null
            }
            </div>
